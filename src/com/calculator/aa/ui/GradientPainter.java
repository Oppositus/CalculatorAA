...
class GradientPainter {
    ...
    enum ColorName {Begin, Middle, End}
    ...
    GradientPainter(Color c0, Color c50, Color c100) {
        color0 = c0;
        color50 = c50;
        color100 = c100;
    }

    void updateColor(ColorName name, Color c) {
        switch (name) {
            case Begin:
                color0 = c;
                break;
    ...
    void paintBackground(Component c, Graphics g, Rectangle clip) {
        int x = 0;
        int y = 0;
        int w = c.getWidth();
        int h = c.getHeight();
